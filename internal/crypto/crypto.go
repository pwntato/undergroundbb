...
//
// Implementations arrive in M2 (crypto core), tested against fixed vectors
// shared with the TypeScript implementation. Round-trip tests are not enough:
// they pass even when both sides are wrong in the same way, and a silent change
// to key derivation would lock every existing user out permanently.
package crypto
