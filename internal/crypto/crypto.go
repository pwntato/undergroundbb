...
//
// The primitives here (AES-256-GCM, Ed25519, X25519 key wrapping) will be
// tested against fixed vectors shared with the TypeScript implementation
// (#20, #24) once those land. Round-trip tests are not enough on their own:
// they pass even when both sides are wrong in the same way, and a silent
// change to key derivation would lock every existing user out permanently.
package crypto
